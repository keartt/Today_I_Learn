window.onload = function() {
	...
	for (let checkBoxElement of checkBoxElements) {
		checkBoxElement.addEventListener("change", function(event) {
			...
			if (event.target.value == 'te_scco_sid') {
			...
			} else if (event.target.value == 'te_scco_sgg_ln') {
				checkLayer = wmslist[event.target.value];
				this.checked ?
					(checkLayer.visible = true) :
					(checkLayer.visible = false);
			...
			} else if (event.target.value == 'testlayer1') {
				checkLayer = wmslist[event.target.value];
				this.checked ?
					(checkLayer.visible = true) :
					(checkLayer.visible = false);
	...
	coordinateToggle.addEventListener('change', () => {
		if (coordinateToggle.checked) {
			clickCallback = function(evt) {
				...
				// Ajax 요청 보내기
				$.ajax({
					url: '/test01/save-coordinate', // 저장할 데이터를 처리할 서버 스크립트 경로
					type: 'POST', // 데이터 전송 방식
